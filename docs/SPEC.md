...
- **Processamento:**
  ...
  - Geração de embeddings via `GoogleGenerativeAIEmbeddings` (modelo: `models/embedding-001`)
...
- **Processamento:**
  ...
  4. Chamar LLM (`gemini-2.5-flash-lite`) com o prompt montado
...
| Resultados k    | 10                                              |
| Variáveis env   | `GOOGLE_API_KEY`, `DATABASE_URL`                |
...
├── .env.example              # Template: GOOGLE_API_KEY e DATABASE_URL
...
```
chat.py  →  services/  →  config/  →  [PostgreSQL | Gemini API]
```
